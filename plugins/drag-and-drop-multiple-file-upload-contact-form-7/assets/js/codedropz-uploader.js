	...
	$.fn.CodeDropz_Uploader = function( settings ){
			...
			var DND_Setup_Uploader = function( files, action ) {
				...
				$.each( files,function( i, file ) {
					...
					if( file.size > options.max_upload_size ) {
						$('.dnd-upload-details', $('#' + progressBarID)).append('<span class="has-error d-none">'+ dnd_cf7_uploader.drag_n_drop_upload.large_file +'</span>');
						options_handler.append('<span class="has-error-msg" id="rekki-error">'+ dnd_cf7_uploader.drag_n_drop_upload.large_file +'</span>');
		...
		$(document).on("click",'.dnd-icon-remove',function(e){
			...
			if( $( '.has-error', _dnd_status ).length > 0 ) {
				_rekki_error.remove();_dnd_status.remove(); localStorage.setItem( removeStorageData, ( storageCount - 1 ) );
				return false;
			}
