...
class DUPX_ServerConfig
{
    ...
    public static function reset($rootPath)
    {
        ...
        if (DupProSnapLibIOU::chmod($rootPath, 'u+rwx') === false) {
            DUPX_Log::info("RESET CONFIG FILES: chmod error on root path ".$rootPath);
        }

        switch ($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_WP_CONFIG)) {
            case 'modify':
            case 'new':
                self::runReset($rootPath.'/wp-config.php', self::CONFIG_ORIG_FILE_WPCONFIG_ID);
                break;
            case 'nothing':
                break;
        }
        ...
        switch ($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_HTACCESS_CONFIG)) {
            case 'new':
            case 'original':
                self::runReset($rootPath.'/.htaccess', self::CONFIG_ORIG_FILE_HTACCESS_ID);
                break;
            case 'nothing':
                break;
        }
        ...
        switch ($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_OTHER_CONFIG)) {
            case 'new':
            case 'original':
                self::runReset($rootPath.'/web.config', self::CONFIG_ORIG_FILE_WEBCONFIG_ID);
                self::runReset($rootPath.'/.user.ini', self::CONFIG_ORIG_FILE_USERINI_ID);
                self::runReset($rootPath.'/php.ini', self::CONFIG_ORIG_FILE_PHPINI_ID);
                break;
            case 'nothing':
                break;
        }

        DUPX_Log::info("*** RESET CONFIG FILES OK");
    }
    ...
    private static function runReset($filePath, $storedName)
    {
        ...
        if (file_exists($filePath)) {
            if (DupProSnapLibIOU::chmod($filePath, 'u+rw') === false) {
                DUPX_Log::info("RESET CONFIG FILES: chmod error on file config path ".$filePath);
            }

            $origFiles = DUPX_Orig_File_Manager::getInstance();
            $filePath  = DupProSnapLibIOU::safePathUntrailingslashit($filePath);

            DUPX_Log::info("I'M GOING TO MOVE CONFIG FILE ".DUPX_LOG::varToString($fileName)." IN ORIGINAL FOLDER");
            ...
            if ($origFiles->addEntry(self::INSTALLER_HOST_ENTITY_PREFIX.$storedName, $filePath, DUPX_Orig_File_Manager::MODE_MOVE, self::INSTALLER_HOST_ENTITY_PREFIX.$storedName)) {
                ...
                DUPX_Log::info("\tRESET ERROR can\'t stored in orginal file folder");
                return false;
            }
        } else {
