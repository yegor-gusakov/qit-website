...
$content_width = 600 - $options['block_padding_left']
                 - $options['block_padding_right'];
$title_style   = TNP_Composer::get_title_style( $options, 'title', $composer );
$text_style    = TNP_Composer::get_style( $options, '', $composer, 'text' );
?>
<style>
    .title {
    <?php echo $title_style->echo_css() ?> line-height: normal !important;
        padding: 0 0 5px 0;
    }

    .excerpt {
    <?php echo $text_style->echo_css() ?> line-height: 1.5em !important;
        padding: 10px 0 15px 0;
    }

    .meta {
    <?php echo $text_style->echo_css(0.9) ?> line-height: normal !important;
        padding: 0 0 5px 0;
        font-style: italic;
    }
</style>
...
<?php foreach ( $posts as $post ) { ?>
	...
	if ( $show_rekki_tags ) {
		$post_term_tags_ = get_the_terms( $post->ID, 'qit_posts_tags' );
		if ( $post_term_tags_ ) {
	...
	if ( $show_rekki_min_read ) :
		$post_rekki_min_read = get_post_meta( $post->ID, 'minutes_to_read' );
            ...
            <td>
                ...
                <table border="0" cellspacing="0" cellpadding="0" width="100%">
                    ...
                    <tr>
                        ...
                        <td align="left">
			                <?php foreach (
				                $post_term_tag as $post_term_tag_name
			                ):
				                ?>
                                <span class="tag"
                                      style="font-weight: 400;font-size: 14px;line-height: 20px;color: #363636;opacity: 0.5;"><?= $post_term_tag_name ?></span>
