...
class TNP_Composer {
    ...
    static function button($options, $prefix = 'button') {
        ...
        $b = '<table class="rekki_button" border="0" cellpadding="0" cellspacing="0" role="presentation" style="margin: 0 auto;position:absolute;bottom:32px; right:32px;"';
        ...
        $b .= '>';
        $b .= '<tr>';
        $b .= '<td align="center" bgcolor="' . $options[$prefix . '_background'] . '" role="presentation" style="border:none;border-radius:59px;cursor:auto;mso-padding-alt:10px 25px;background:' . $options[$prefix . '_background'] . '" valign="middle">';
        $b .= '<button ';
        $b .= ' style="display:inline-block;background:' . $options[$prefix . '_background'] . ';color:' . $options[$prefix . '_font_color'] . ';font-family:' . $options[$prefix . '_font_family'] . ';font-size:' . $options[$prefix . '_font_size'] . 'px;font-weight:' . $options[$prefix . '_font_weight'] . ';line-height:120%;margin:0;text-decoration:none;text-transform:none;padding:10px 25px;mso-padding-alt:0px;border-radius:59px;"';
        $b .= ' >';
    ...
    static function image($media, $attr = []) {
        ...
        $b = '';
        if ($media->link) {
            $b .= '<a href="' . esc_attr($media->link) . '" target="_blank" rel="noopener nofollow" style="width:100%; display: inline-block; font-size: 0; text-decoration: none; line-height: normal!important">';
        } else {
        ...
        if ($media) {
            $b .= '<img style="width:100%;display:block;border-radius:15px!important;" src="' . esc_attr($media->url) . '" width="' . esc_attr($media->width) . '"';
            if ($media->height) {
    ...
    static function grid($items = [], $attrs = []) {
        $attrs = wp_parse_args($attrs, ['width' => 950, 'columns' => 2, 'padding' => 10, 'responsive' => true]);
        ...
        if ($attrs['responsive']) {
            ...
            foreach ($chunks as &$chunk) {
                ...
                foreach ($chunk as &$item) {
                    ...
                    $e .= '<div class="max-width-100" style="width:100%;max-width:' . $column_width . ';display:inline-block;vertical-align: top;box-sizing: border-box;">';
